...
async function finalizePreparedV2WorkerStartup(prepared, options = {}) {
    ...
    if (prepared.agentType === 'claude') {
        const settled = await waitForWorkerStartupEvidence(prepared.teamName, prepared.workerName, prepared.taskId, prepared.cwd);
        if (!settled) {
            ...
            const settledAfterRetry = await waitForWorkerStartupEvidence(prepared.teamName, prepared.workerName, prepared.taskId, prepared.cwd);
...
export async function startTeamV2(config) {
    ...
    const failedPrelaunchWorkers = preparedWorkerLaunches.filter((workerLaunch) => !workerLaunch.paneId || workerLaunch.startupFailureReason);
    ...
    const readyPhaseWorkers = preparedWorkerLaunches.filter((workerLaunch) => Boolean(workerLaunch.paneId) && !workerLaunch.startupFailureReason);
    ...
    const pendingInteractiveStarts = new Map(interactivePrepared.map((prepared) => [
        prepared.workerName,
    ...
    while (pendingInteractiveStarts.size > 0) {
        ...
        const startupResult = paneReady
            ? await finalizePreparedV2WorkerStartup(prepared, { paneReady: true })
            : {
